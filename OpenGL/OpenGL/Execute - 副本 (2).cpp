...
int main(int, char**)
{
  ...
  /*  for (int i = 0; i < control.size(); i++)
    ...
    particleImage1.Bind();
    TextureImage particleImage2 = TextureImage(Input_N, Input_N);
    particleImage2.Bind();
    ...
    glm::mat4 trans= glm::mat4(1.0f);
     ...
     trans = glm::rotate(trans, glm::radians(20.f), glm::vec3(0.0f, 0.0f, 1.0f));
  
    myshader.setMatrix("transform", trans);
    lightshader.setMatrix("transform", trans);
    InstanceShader.setMatrix("transform", trans);
    glm::vec3 lightPos(0.0f, 10.0f, 4.0f);
    glm::vec3 lightPos1(0.0f, 3.0f, 2.0f);
    glm::vec3 lightDir=lightPos;
    glm::vec4 lightColor(1, 1, 1,0);
   
    Light light(DIRLIGHT, glm::vec3(0.0, 3.0, 1.0), -glm::vec3(0.0, 0.4, 0.5), lightColor);
   ...
   // pool.PushToParticles(glm::vec2(512, 512), glm::vec2(512, 512), glm::vec2(0.005*1024, 0.005 * 1024), 0.1f, 5.0f, 60, 0.0);
     ...
     /*   glEnable(GL_BLEND);
      ...
      //  model = glm::rotate(model, glm::radians(70.f*(float)glfwGetTime()), glm::vec3(0.0f, 1.0f, 0.0f));
       ...
       // sponzaModel.Draw(depthShader);
        depthShader.setMatrix("model", floormodel);
        roomModel.Draw(depthShader);
        depthMap.BindFrameBufeerOver();
        std::vector<Texture> rt = std::vector<Texture>(6);
            ...
            if(!enableWaveParticle)
             {
                ...
                normalShader.setFloat("resolutioninv", 2.0f/(float)256);//平面的长度为2
                ...
                /*     blit4.BindFrameBufferInit();
                     ...
                     blit3.shader.setFloat("Time", Time);
                     glBindTexture(GL_TEXTURE_2D, temp3.texture.id);
                     blit3.DrawQuad(temp2.texture.id, false);
                     blit3.Blit(temp1);
                     blit3.BindFrameBufferOver();*/
                ...
                normalShader1.setFloat("resolutioninv", 2.0f/1024.0f);
                normalShader1.dispatch();
                normalShader1.wait();
            }
         /*   for (int i = 0; i < 3; i++) {
                GaussianBlur_h1.texes_input[0] = normalShader1.texes_output[0];
                GaussianBlur_h1.use();
                GaussianBlur_h1.dispatch();
                GaussianBlur_h1.wait();

                GaussianBlur_v1.texes_input[0] = GaussianBlur_h1.texes_output[0];
                GaussianBlur_v1.use();
                GaussianBlur_v1.dispatch();
                GaussianBlur_v1.wait();
                normalShader1.texes_output[0] = GaussianBlur_v1.texes_output[0];
            }
         */
          //  temp1.texture.id = normalShader1.texes_output[0].id;
           /* for (int i = 0; i < 7; i++)
            {
                ...
                if (i == 0) 
                {
                    ...
                    blit1.UseShader();
                    blit1.shader.setFloat("texelsize_v", 2.0f / (float)height);
                    blit1.DrawQuad(normalShader1.texes_output[0].id, false);
                    blit1.Blit(temp1);
                    blit1.BindFrameBufferOver();
                }
                else
            ...
            normalShader1.texes_output[0].id = temp2.texture.id;*/
          /*  for (int i = 0; i < 3; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    blit1.DrawQuad(normalShader1.texes_output[1].id, false);
                    blit1.Blit(temp1);
                    blit1.BindFrameBufferOver();
                }
                else
            ...
            normalShader1.texes_output[1].id = temp3.texture.id;*/
         ...
         posteffect.DebugDraw(wavemap.texture.id);
       ...
       posteffect.Draw();
        ...
        if (showwindow)
        {
            ...
            ImGui::SliderFloat("WaveScale", (float*)&(scale_Wave), 0.0f, 1.0f);
